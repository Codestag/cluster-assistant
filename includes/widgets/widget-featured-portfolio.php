...
add_action( 'widgets_init', function() {
	return register_widget( 'stag_featured_portfolio' );
} );
...
class stag_featured_portfolio extends WP_Widget {
	function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Display a featured portfolio', 'cluster-assistant' ),
		);
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'stag_featured_portfolio' );
		parent::__construct( 'stag_featured_portfolio', __( 'Featured Portfolio', 'cluster-assistant' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'postid' ); ?>"><?php esc_html_e( 'Post ID:', 'cluster-assistant' ); ?></label>
			...
			<span class="description"><?php esc_html_e( 'Enter the Portfolio post ID', 'cluster-assistant' ); ?></span>
		</p>
