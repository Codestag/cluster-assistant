...
function stag_admin_styles( $hook ) {
	if ( 'post.php' === $hook || 'post-new.php' === $hook ) {
		wp_register_style( 'stag-admin-metabox', CA_PLUGIN_URL . '/assets/css/stag-admin-metabox.css', array( 'wp-color-picker' ) );
...
function stag_admin_scripts( $hook ) {
	if ( 'post.php' === $hook || 'post-new.php' === $hook || 'widgets.php' === $hook ) {
		wp_enqueue_media();
		wp_register_script( 'stag-admin-metabox', CA_PLUGIN_URL . '/assets/js/stag-admin-metabox.js', array( 'jquery', 'wp-color-picker' ) );
				...
				jQuery(function($){
					...
					$('#<?php echo $field['id']; ?>_button').on('click', function(e) {
						...
						frame.toolbar.get('view').set({
							insert: {
								style: 'primary',
								text: '<?php _e("Insert", 'cluster-assistant'); ?>',
			...
			case 'images':
			    ...
			    jQuery(function($){
			        ...
			        $('#stag_images_upload').on('click', function(e) {
			            e.preventDefault();
			            var options = {
			                title: '<?php _e("Create Featured Gallery", 'cluster-assistant'); ?>',
			                state: 'gallery-edit',
			                frame: 'post',
			                selection: selection
			            };
			            ...
			            if( frame || selection ) {
			                options['title'] = '<?php _e("Edit Featured Gallery", 'cluster-assistant'); ?>';
			            }
			            ...
			            frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e("Edit Featured Gallery", 'cluster-assistant'); ?>';
			            ...
			            function overrideGalleryInsert(){
			                frame.toolbar.get('view').set({
			                    insert: {
			                        style: 'primary',
			                        text: '<?php _e("Save Featured Gallery", 'cluster-assistant'); ?>',
			                        click: function(){
			                            var models = frame.state().get('library'),
			                                ids = '';
			                            ...
			                            this.el.innerHTML = '<?php _e("Saving...", 'cluster-assistant'); ?>';
			    ...
			    $thumbs_output = '';
			    $button_text = ($meta) ? __('Edit Gallery', 'cluster-assistant') : $field['std'];
			    if( $meta ) {
			        $field['std'] = __('Edit Gallery', 'cluster-assistant');
			        $thumbs = explode(',', $meta);
			        $thumbs_output = '';
